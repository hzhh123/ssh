                    ...
                    <div class="col-xs-12">
                        ...
                        <div class="col-sm-9">
                            ...
                            <div class="btn-group pull-right">
                                ...
                                <button class="btn btn-warning btn-white"><i class="fa fa-file-excel-o  bigger-110 orange"></i> 导入</button>
                            </div>
                            <div class="layui-row">
                                <table  class="layui-table" lay-data="{id:'dataGrid',height:'full-250', url:'${ctxPath}/user/user!dataGrid',loading:true,page:true,limits: [30,60,90,150,300,500,1000]}" lay-filter="test">
                                    <thead>
                                    <tr>
                                        ...
                                        <th lay-data="{field:'name', width:80}">姓名</th>
                                         ...
                                         <th lay-data="{field:'workType', width:100}">职务/工种</th>
                                        ...
                                        <th lay-data="{field:'userType', width:90}">用户类型</th>
                                        <th lay-data="{field:'roleNameList', width:100}">角色</th>
                                        <!-- <th lay-data="{field:'roleList', width:100}">角色</th> -->
                                        <th lay-data="{field:'national', width:80}">民族</th>
                                        <th lay-data="{field:'education', width:100}">文化程度</th>
                                        <th lay-data="{field:'age', width:80,sort: true}">年龄</th>
                                        <th lay-data="{field:'birthday', width:90}">出生日期</th>
                                        <th lay-data="{field:'idNum', width:90}">身份证号码</th>
...
function add() {
    layer.open({
        ...
        maxmin: true,
        area: ['90%', '90%'],
        content: '${ctxPath}/user/user!addPage' //iframe的url
    });
}
    jQuery(function($) {
        ...
        layui.use('table', function(){
            ...
            table.on('tool(test)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                ...
                if(layEvent === 'detail'){ //查看
                        layer.open({
                            ...
                            maxmin: true,
                            area: ['50%', '90%'],
                            content:"${ctxPath}/user/user!viewPage?id="+data.id,
                        });
                    ...
                    layer.open({
                        ...
                        maxmin: true,
                        area: ['50%', '90%'],
                        content:"${ctxPath}/user/user!editPage?id="+data.id,
                    });
                }
            });
            var $ = layui.$, active = {
                getCheckData: function(){ //获取选中数据
                    ...
                    if(data.length==0){
                        ...
                        layer.confirm('真的删除这'+data.length+'条记录吗?', function(index){
                            ...
                            $.ajax({
                                url:'${ctxPath}/user/user!deleteBatch',
                                data:{"ids":ids.join(',')},
                                dataType:'json',
                                type:'post',
                                success:function(data){
                                        layer.msg("删除成功！",{icon:1,time:1000});
                                        table.reload('dataGrid',{url:'${ctxPath}/user/user!dataGrid'});
                                },
                                error: function(XmlHttpRequest, textStatus, errorThrown){
                                    layer.msg('删除失败!',{icon:5,time:1000});
                                }
            ...
            $('#deptTree').jstree({
                ...
                var organizationId=selected.node.id;
                console.log(event)
                layui.use('table', function() {
                    var table = layui.table;
                    table.reload('dataGrid', {
                        url: '/user/user!dataGrid'
            ...
            $("#deptTree").parent().niceScroll({
          	...
          	//设置菜单激活
            var href=basePath+"/jsp/user/userList.jsp";
